<template>
  <div
    ref="star"
    class="absolute bg-white rounded-full opacity-100 star"
    :style="style"
  />
</template>
...
export default defineComponent({
  setup() {
    ...
    const randomRadius = computed(() =>
      Math.round(Math.random() * Math.round(4))
    );
    const randomDuration = computed(() =>
      Math.round(Math.random() * Math.round(10))
    );
